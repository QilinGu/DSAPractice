...
def findFraudolentTraders(datafeed):
    ...
    for feed in datafeed:
        ...
        if len(vals) == 2:
            ...
            checkTrades(day, insider_trades, trades, current_price)
        else:
            saveTradeRecord(vals, day, trades, current_price)
    insider_trades = sorted(list(insider_trades))
    # return list(map(lambda x: str(x[0]) + "|" + str(x[1]), flagged_trades))
    return [ str(x[0]) + "|" + str(x[1]) for x in insider_trades ]

def checkTrades(day, insider_trades, trades, current_price):
    # current_price = int(vals[1])
    for x in range(day - 3, day):
        if x in trades:
            for (trader_name, isBuy, price, amount) in trades[x]:
                if (x, trader_name) in insider_trades:
                    continue
                if isBuy:
                ...
                if fraudolent:
                    insider_trades.add((x, trader_name))

def saveTradeRecord(vals, day, trades, current_price):
    trader_name = vals[1]
    isBuy = len(vals[2]) == 3
    amount = int(vals[3])
    record = (trader_name, isBuy, current_price, amount)
    trades[day] = trades.get(day, []) + [record]
